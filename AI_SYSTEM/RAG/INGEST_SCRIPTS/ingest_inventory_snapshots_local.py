...
# ========== CONFIGURATION ==========
DATA_FOLDER = r"C:\Users\Sandeep\Desktop\roborana_ai_system\RoboRana_AI_Data\DATA\INVENTORY\FINAL"
...
if not csv_files:
    raise FileNotFoundError("❌ No CSV files found in inventory folder.")

print(f"\n📂 Found {len(csv_files)} inventory snapshots. Reading and combining...\n")
...
for file in tqdm(csv_files):
    try:
        ...
        # Extract snapshot date/time from filename
        snapshot_info = filename.replace("inventory_", "").replace(".csv", "")
...
for i, row in merged_df.iterrows():
    # Combine relevant info as text (you can adjust columns here)
